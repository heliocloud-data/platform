NEW_VERSION=$1
DEST=kube/kubeadm/nvidia-device-plugin/base/daemonset.yaml
...
HTTP_CODE=$(curl \
  --silent \
  --write-out "%{http_code}" \
  https://raw.githubusercontent.com/NVIDIA/k8s-device-plugin/${NEW_VERSION}/deployments/static/nvidia-device-plugin.yml \
  -o ${DEST})
