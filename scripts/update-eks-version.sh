...
if [[ "${SKIP_KUBECTL_URL_UPDATE}" != "true" ]]; then
  if [[ "${NEW_VERSION}" == "" ]]; then
    ...
    if [[ "${NEW_VERSION}" == "" ]]; then
      ...
      echo "usage: ${0} <version> <skip-kubectl-url-update> <no-commit>"
      echo ""
      exit 1
    fi
...
sed \
  -i \
  "s#registry.k8s.io/autoscaling/cluster-autoscaler:.*#registry.k8s.io/autoscaling/cluster-autoscaler:${NEW_CLUSTER_AUTOSCALER_VERSION}#" \
  ${TARGET_FILE}

# Update the kube-scheduler version
TARGET_FILE="kube/apps/daskhub/values.yaml"
echo "Updating file ${TARGET_FILE}..."
sed -i "s#tag: v1..*#tag: ${NEW_KUBE_SCHEDULER_VERSION}#" ${TARGET_FILE}

# # Search for the Amazon EKS URL that's litered throughout the project.
for FILE in `grep -lr 's3.us-west-2.amazonaws.com/amazon-eks' scripts | grep .sh | grep -v release | grep -v jhuapl-operations | grep -v cdk.out | grep -v temp | grep -v .terraform | grep -v $0`; do
