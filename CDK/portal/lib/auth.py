...
# Timeout in seconds to use for HTTPS requests against urls (ex: GET, POST)
# Best practice is always to specify a timeout to avoid indefinitely blocking calls
REQUEST_TIMEOUT = 60


def get_cognito_public_keys() -> str:
    """
...
def get_user_info(access_token) -> tuple:
    """
    ...
    response = requests.post(user_url, headers=header, timeout=REQUEST_TIMEOUT)
    response_json = response.json()
