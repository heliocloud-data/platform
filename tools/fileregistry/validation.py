    ...
    def _fetch_and_combine_catalogs(self, catalog_url: Optional[str] = None) -> None:
        ...
        if catalog_url is None:
            ...
            if catalog_url is None:
                ...
                raise ValueError(
                    "No environment variable ROOT_CATALOG_REGISTRY_URL nor was an explicit catalog_url passed in."
                )
    ...
    def validate_local_uniqueness(self) -> None:
        ...
        for catalog in self.combined_catalog:
            for entry in catalog["catalog"]:
                ...
                if identifier in unique_identifiers:
                    logging.warning(
                        f"{identifier} is not a unique entry. Conflicts in {unique_identifiers[identifier]} and {catalog['name']}"
                    )
                    dups += 1
                else:
