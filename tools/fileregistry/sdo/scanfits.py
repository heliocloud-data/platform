...
def main():
    test_fnames = ['sdo_aia_h2_20121231T235600_0094_v1.fits',
...
def sdo_scanfits(fname):
    try:
        hdul = fits.open(fname, use_fsspec=True,
                         checksum=True,
                         fsspec_kwards={"anon": True})
    ...
    keywords = ['TELESCOP', 'INSTRUME', 'WAVELNTH', 'CAMERA',
                'CONTENT', 'T_OBS', 'QUALITY', 'CHECKSUM',
                'CRLN_OBS', 'CRLT_OBS', 'CAR_ROT']
    if hdul[1].header['TELESCOP'] == 'SDO/AIA':
        keywords += ['WAVE_STR', 'PROC_LEV', 'IMG_TYPE',
                         'EXPTIME', 'LVL_NUM', 'PERCENTD', 'QUALLEV0']
