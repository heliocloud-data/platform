...
def get_weekly_overall_cost(aws_session, username) -> dict:
    """
    ...
    week_cost = 0
    for record in week_resp["ResultsByTime"]:
        week_cost += float(record["Total"]["UnblendedCost"]["Amount"])
    ...
    overall_cost = 0
    for record in overall_resp["ResultsByTime"]:
        overall_cost += float(record["Total"]["UnblendedCost"]["Amount"])
