...
def index():
    ...
    if get_jwt_identity():
        ...
        if request.method=='GET':
            ...
            stopped_instances = [ins for ins in instances if ins['instance_state'] in ['stopped','stopping']]
            # sort stopped and running by launch time and only show last 5 stopped instances
            if len(running_instances)>0:
                running_instances = [ins for ins in sorted(running_instances, key=lambda item: item['LaunchTime'], reverse=True)]
            if len(stopped_instances)>0:
                stopped_instances = [ins for ins in sorted(stopped_instances, key=lambda item: item['LaunchTime'], reverse=True)][:5]
            cost = get_weekly_overall_cost(aws_session, username)

            return render_template('index.html', running_instances=running_instances, stopped_instances=stopped_instances, user=username, cost=cost)
    else:
        ...
        if request.method=='POST':
            ...
            try:
                ...
                if len(running_instances)>=3:
                    return render_template('error.html', error_code='Too many running instances.', error_message=f"You have {len(running_instances)} running instances. Please stop an instance before starting another.")
                else:
                    ...
                    for key,val in instance_launch_info.items():
                        if val==None:
                            need_key.append(key)
                    if len(need_key)>0:
                        return render_template('error.html', error_code='Insufficient parameters', error_message=f'Must choose values for parameters {need_key}.')
                    if instance_launch_info['volume_size']<instance_launch_info['min_volume_size']:
                        return render_template('error.html', error_code='Insufficient volume size',
                        ...
                        if 'Error' in response:
                            ...
                            return render_template('error.html', error_code=error['Code'], error_message=error['Message'])
                        else:
                            return redirect(url_for('index'))
            except Exception as e:
                return render_template('error.html', error_code='Botocore Error', error_message=e)
            ...
            allowed_images_dict = {os: get_ami_info(aws_session, image_list) for os, image_list in
                                   image_id_dict[region].items()}
            ...
            return render_template('launch_instance.html', allowed_images=allowed_images_dict, key_pair_names=key_pair_names, instance_types=instance_types, user=username)
    else:
        return redirect(aws_auth.get_sign_in_url())
    ...
    if get_jwt_identity():
        ...
        if access_flag:
            if request.method=='GET':
                ...
                return render_template('access.html', active_access_keys=active_access_keys, inactive_access_keys=inactive_access_keys, user=username, mfa_devices=mfa_devices, message='', download=False)
            else:
                if request.form.get('create_access_key') is not None:
                    try:
                        ...
                        if 'Error' in resp:
                            ...
                            return render_template('access.html', active_access_keys=active_access_keys, inactive_access_keys=inactive_access_keys, mfa_devices=mfa_devices, user=username, message=access_key_message.format(access_key_id, secret_access_key), download='secret_key')
                    except Exception as e:
                        return render_template('error.html', error_code='Botocore Error', error_message=e)
                if request.form.get('refresh_tokens') is not None:
                    try:
                        ...
                        resp = get_session_token(access_key_id, secret_access_key, mfa_arn, mfa_code)
                        session_token_message = make_session_token_message(resp['AccessKeyId'], resp['SecretAccessKey'], resp['SessionToken'])
                        active_access_keys, inactive_access_keys = list_access_key(aws_session, aws_console_username)
                        mfa_devices = list_mfa_devices(aws_session, aws_console_username)
                        session['download'] = {'access': resp['AccessKeyId'], 'secret': resp['SecretAccessKey'], 'token': resp['SessionToken']}
                        return render_template('access.html', active_access_keys=active_access_keys,
                                               inactive_access_keys=inactive_access_keys, mfa_devices=mfa_devices, user=username,
                                               message=session_token_message, download='session_token')
                    except Exception as e:
                        return render_template('error.html', error_code='Botocore Error', error_message=e)
        else:
            return render_template('error.html', error_code='', error_message='Could not find an associated AWS account. Please contact an administrator to set up your AWS user account.')
    else:
        return redirect(aws_auth.get_sign_in_url())
...
def get_keypairs():
    ...
    if get_jwt_identity():
        ...
        if request.method=='GET':
            ...
            return render_template('keypairs.html', keypairs=keypairs, user=username, message='', download=False)
        else:
            if request.form.get('create_keypair') is not None:
                ...
                try:
                    ...
                    if 'Error' in resp:
                        ...
                        keypairs = list_key_pairs(aws_session, username)
                        session['download'] = {'name':keypair_name, 'material':key_material}
                        return render_template('keypairs.html', keypairs=keypairs, user=username, message=keypair_message.format(keypair_name, keypair_name), download=True)
                except Exception as e:
                    return render_template('error.html', error_code='Botocore Error', error_message=e)
                ...
                try:
                    ...
                    keypairs = list_key_pairs(aws_session, username)
                    return render_template('keypairs.html', keypairs=keypairs, user=username, message=message, download=False)
                except Exception as e:
                    return render_template('error.html', error_code='Botocore Error', error_message=e)
    else:
        return redirect(aws_auth.get_sign_in_url())
