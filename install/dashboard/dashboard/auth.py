...
from config import aws_cognito_domain, redirect_url, user_pool_client_id, user_pool_client_secret, user_pool_id, region
import json
...
def get_tokens(request_args):
    ...
    secret = b64encode(
                f"{user_pool_client_id}:{user_pool_client_secret}".encode(
                    "utf-8"
                )
            ).decode("utf-8")
