...
class WebRetrievalStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        check_file_registry_lambda = lambda_.Function(
            self,
            "check_file_registry_lambda",
        ...
        data_load_parse_small_lambda = lambda_.Function(
            self,
            "data_load_parse_small_lambda",
            runtime=lambda_.Runtime.PYTHON_3_9,
        ...
        data_load_parse_medium_lambda = lambda_.Function(
            self,
            "data_load_parse_medium_lambda",
            runtime=lambda_.Runtime.PYTHON_3_9,
            handler="data_load.lambda_handler",
            code=lambda_.Code.from_asset("./web_retrieval/lambdas"),
            timeout=Duration.minutes(10),
        ...
        data_load_parse_large_lambda = lambda_.Function(
            self,
            "data_load_parse_large_lambda",
            runtime=lambda_.Runtime.PYTHON_3_9,
            handler="data_load.lambda_handler",
            code=lambda_.Code.from_asset("./web_retrieval/lambdas"),
            timeout=Duration.minutes(10),
        ...
        log_failures_lambda = lambda_.Function(
            self,
            "log_failures_lambda",
            runtime=lambda_.Runtime.PYTHON_3_9,
        ...
        failed_job = sfn.Fail(self, "Failed Job", cause="Failed Job", error="did not parse chunk")

        check_registry_job = sfn_tasks.LambdaInvoke(
            self,
            "check registry Job",
        ...
        log_failures_job = sfn_tasks.LambdaInvoke(
            self, "log failures Job", lambda_function=log_failures_lambda, output_path="$.Payload"
        ).next(failed_job)

        data_load_parse_large_job = sfn_tasks.LambdaInvoke(
            self,
            "data load parse large Job",
            lambda_function=data_load_parse_large_lambda,
            output_path="$.Payload",
        )
        data_load_parse_large_job.add_catch(result_path="$.error", handler=log_failures_job).next(
            succeed_job
        )

        data_load_parse_medium_job = sfn_tasks.LambdaInvoke(
            self,
            "data load parse medium Job",
            lambda_function=data_load_parse_medium_lambda,
            output_path="$.Payload",
        )
        data_load_parse_medium_job.add_catch(
            result_path="$.error", handler=data_load_parse_large_job
        ).next(succeed_job)

        data_load_parse_small_job = sfn_tasks.LambdaInvoke(
            self,
            "data load parse small Job",
            lambda_function=data_load_parse_small_lambda,
            output_path="$.Payload",
        )
        data_load_parse_small_job.add_catch(
            result_path="$.error", handler=data_load_parse_medium_job
        ).next(succeed_job)

        data_upload_definition = check_registry_job.next(data_load_parse_small_job)

        data_upload_sm = sfn.StateMachine(
            self, "Data Upload SM", definition=data_upload_definition, timeout=Duration.seconds(30)
        )

        ############

        # Lambdas for start full pipeline step functions
        chunk_file_lambda = lambda_alpha.PythonFunction(
            self,
            "chunk_file_lambda",
        ...
        make_upload_lambda = lambda_alpha.PythonFunction(
            self,
            "make_upload_lambda",
            entry="./web_retrieval/lambdas/chunk_files",
            index="make_upload_json.py",
            environment={"SAVE_S3_BUCKET": s3_save_data.bucket_name},
            runtime=lambda_.Runtime.PYTHON_3_9,
            handler="lambda_handler",
        )
        ...
        data_upload_sm_job = sfn_tasks.StepFunctionsStartExecution(
            self,
            "data upload sm job",
        ...
        next_step_function_job = sfn.Map(
            self,
            "Test step function",
            max_concurrency=5,
        ...
        chunk_file_job = sfn_tasks.LambdaInvoke(
            self, "chunk file Job", lambda_function=chunk_file_lambda, output_path="$.Payload"
        )

        make_upload_job = sfn_tasks.LambdaInvoke(
            self, "make upload Job", lambda_function=make_upload_lambda, output_path="$.Payload"
        )

        chunk_manifest_job = sfn.Map(
            self,
            "Chunk Manifest",
            max_concurrency=1,
            items_path=sfn.JsonPath.string_at("$.chunked_files"),
        )
        chunk_manifest_job.iterator(make_upload_job.next(next_step_function_job))

        full_pipeline_definition = chunk_file_job.next(chunk_manifest_job)

        # Step function to kick off full pipeline
        full_pipeline_sm = sfn.StateMachine(
            self, "Full processing pipeline", definition=full_pipeline_definition
        )
        ...
        start_full_pipeline_function = lambda_.Function(
            self,
            "start_full_pipeline_function",
            runtime=lambda_.Runtime.PYTHON_3_9,
            handler="start_full_pipeline.lambda_handler",
            code=lambda_.Code.from_asset("./web_retrieval/lambdas"),
            environment={"STEPFUNCTION_ARN": full_pipeline_sm.state_machine_arn},
        ...
        s3_manifest.add_event_notification(s3_.EventType.OBJECT_CREATED, reprocessing_notification)
