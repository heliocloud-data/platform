...
log_stream = StringIO()
logging.basicConfig(
    stream=log_stream, level=logging.ERROR, format="%(asctime)s:%(levelname)s:%(message)s"
)
...
class DataLoaderReader:
    ...
    def download_file(self):
        ...
        response = self.s3.upload_file(
            os.path.join("/tmp/fileobj.{}".format(self.file_meta["file_type"])), bucket, filename
        )
    ...
    def read_register_file(self, file_loaded=True):
        ...
        if file_loaded == False:  # pull the file into the tmp directory
            ...
            self.s3.download_file(
                s3_bucket, s3_filename, "/tmp/fileobj.{}".format(self.file_meta["file_type"])
            )
        ...
        if self.file_meta["file_type"] in self.get_supported_file_types:
            ...
            logging.error(
                "File type is not currently supported. Supported file types = {}".format(
...
def lambda_handler(event, context):
    ...
    return {"statusCode": 200, "file_meta": file_meta, "logging_stream": log_stream.getvalue()}
