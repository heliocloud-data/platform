...
def check_registry(bucket, key):
    ...
    response = filereg_table.get_item(Key={"s3_filekey": key})
    ...
    if "Item" in response:  # should only be one item present in the response
        ...
        filereg_response = filereg_table.query(
            IndexName="dataset_datesort", KeyConditionExpression=Key("dataset").eq(dataset)
        )
        ...
        if "Items" in filereg_response:
            ...
            if (
                item_content
            ):  # if there are any contents left in the file registry then update the summary table
                # edit the entry for the summary table item but don't delete the summary table
                summary_response = summary_table.get_item(
                    Key={"mission": mission, "dataset": dataset}
                )
                ...
                summary_table.delete_item(Key={"mission": mission, "dataset": dataset})
