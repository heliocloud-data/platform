# TODO:  Update to Python 3.10 to get more annotations
...
class DataSetRepository(object):
    ...
    def __init__(self, db_client: MongoClient, db="catalog", collection="datasets"):
        """
        Initialize a DataSetRepository instance connected to the
        AWS DocumentDB instance in db_client.
        """
        self.__db_name = db
        self.__catalog_db = db_client.get_database(db)
        self.__datasets_collection = self.__catalog_db.get_collection(collection)

    def save(self, datasets: list[DataSet]) -> int:
        """
        Save a list of Datasets to the repository. Returns nunber of documents saved successfully.
    ...
    def db(self):
        """
        Return name of the database used by this repository.
        """
