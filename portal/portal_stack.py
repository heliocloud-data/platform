...
class PortalStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, config: dict, base_aws: BaseAwsStack, base_auth: Stack, **kwargs) -> None:
        ...
        authenticated_policy = iam.Policy(self, "PortalAuthPolicy", document=authenticated_policy_document)
        authenticated_role = iam.Role(self, "PortalAuthRole",
        ...
        unauthenticated_policy = iam.Policy(self, "PortalUnauthPolicy", document=unauthenticated_policy_document)
        unauthenticated_role = iam.Role(self, "PortalUnauthRole",
            assumed_by=iam.FederatedPrincipal('cognito-identity.amazonaws.com')
        )
        unauthenticated_role.attach_inline_policy(unauthenticated_policy)

        ## Create identity pool
        portal_identity_pool = identity_pool.IdentityPool(self, "IdentityPool",
              ...
              authentication_providers=identity_pool.IdentityPoolAuthenticationProviders(
                    user_pools=[
                        identity_pool.UserPoolAuthenticationProvider(
                            user_pool=base_auth.userpool,
                            user_pool_client=portal_client
                        )
                    ]
              ),
        ...
        portal_ec2_default_sg = ec2.SecurityGroup(self,
        ...
        portal_ec2_default_role = iam.Role(self, "PortalEc2Role",
        ...
        portal_asset = ecr_assets.DockerImageAsset(self,
                                                   "PortalDocker",
                                                   directory="./portal",
                                                   file="./portal/Dockerfile"
                                                   )
        portal_task = ecs.FargateTaskDefinition(self, "PortalFargateTask", cpu=256, memory_limit_mib=512)
        portal_task.add_container(
            ...
            secrets={
                ...
                "USER_POOL_CLIENT_SECRET": ecs.Secret.from_secrets_manager(user_pool_client_secret_secret),
        ...
        portal_hosted_zone = route53.PublicHostedZone.from_lookup(self, "PortalHostedZone", domain_name=portal_config.get('domain_url'))
        if portal_hosted_zone.is_resource(self):
            portal_hosted_zone = route53.PublicHostedZone(self, "PortalHostedZone",
                zone_name=f"{portal_config.get('domain_url')}"
            )
        portal_security_group = ec2.SecurityGroup(self, "PortalSecurityGroup", vpc=base_aws.heliocloud_vpc,
                                                  allow_all_outbound=True)
        portal_certificate = cm.Certificate.from_certificate_arn(self, "domainCert", portal_config.get('domain_certificate_arn'))
        portal_service = ecs_patterns.ApplicationLoadBalancedFargateService(self, "PortalFargate",
